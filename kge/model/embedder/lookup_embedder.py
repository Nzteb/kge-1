...
class LookupEmbedder(KgeEmbedder):
    ...
    def add_embeddings(self, num_new):
        """Adds a new set of embeddings.

        :param num_new: number of new entities/embeddings to add.

        """

        if self.is_augmented:
            raise Exception("Adding new embeddings is not permitted.")

        self._embeddings_new = torch.nn.Embedding(
            num_new, self.dim, sparse=self.sparse
        )
        ...
        if init_ == "uniform_" and "a" not in init_args:
            init_args["a"] = init_args["b"] * -1
            self.set_option("initialize_args.a", init_args["a"], log=True)
        self.initialize(self._embeddings_new.weight.data, init_, init_args)
        ...
        self.vocab_size = old_vocab_size + num_new
